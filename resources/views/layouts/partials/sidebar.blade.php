    ...
    <nav class="flex-1 p-4 space-y-1.5 overflow-y-auto sidebar-scroll">
        ...
        @if(auth()->user()->hasPermission('akses_skpd') && !in_array(strtolower(auth()->user()->role), ['admin', 'administrator', 'superadmin']))
            ...
            <a href="{{ route('surat-tugas.index') }}" 
               class="flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-all {{ request()->routeIs('surat-tugas.*') ? 'bg-blue-600 text-white shadow-md shadow-blue-500/20' : 'hover:bg-slate-800 hover:text-white' }}">
