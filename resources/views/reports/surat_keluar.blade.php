...
<div class="p-4 sm:p-6 lg:p-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800 tracking-tight">Laporan Surat Keluar</h2>
            <p class="text-sm text-slate-500 mt-1">Laporan rekapitulasi data surat keluar beserta filternya.</p>
        </div>
    </div>

    <!-- Filter Form -->
    <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
        <form method="GET" action="{{ route('laporan.surat-keluar') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            <div>
                <label class="block text-xs font-semibold text-slate-700 mb-1">Tanggal Awal</label>
            ...
            <div>
                <label class="block text-xs font-semibold text-slate-700 mb-1">Tujuan</label>
                <select name="tujuan" class="w-full text-sm border-slate-300 rounded-lg focus:border-blue-500 focus:ring-blue-500">
                    <option value="">-- Semua Tujuan --</option>
                    @foreach($tujuanList as $t)
                        <option value="{{ $t }}" {{ request('tujuan') == $t ? 'selected' : '' }}>{{ $t }}</option>
                    @endforeach
                </select>
            </div>
            <div>
            ...
            <div class="md:col-span-4 flex gap-2 justify-end mt-2">
                <a href="{{ route('laporan.surat-keluar') }}" class="px-4 py-2 bg-slate-100 text-slate-600 hover:bg-slate-200 rounded-lg text-sm font-semibold transition-colors">Reset</a>
                ...
                <a href="{{ route('laporan.surat-keluar.pdf', request()->all()) }}" target="_blank" class="px-4 py-2 bg-emerald-600 text-white hover:bg-emerald-700 rounded-lg text-sm font-semibold transition-colors shadow-sm shadow-emerald-500/30 flex items-center gap-2">
                        ...
                        <tr class="hover:bg-slate-50/80 transition-colors">
                            ...
                            <td class="py-4 px-6">
                                @if(strtolower($item->status) == 'draft')
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-slate-50 text-slate-700 border border-slate-200">Draft</span>
                                @elseif(strtolower($item->status) == 'menunggu_dirut')
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-yellow-50 text-yellow-700 border border-yellow-200">Menunggu Dirut</span>
                                ...
                                @elseif(strtolower($item->status) == 'revisi')
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-red-50 text-red-700 border border-red-200">Revisi</span>
                                @else
