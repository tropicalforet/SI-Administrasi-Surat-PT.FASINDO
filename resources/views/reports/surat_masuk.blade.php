...
<div class="p-4 sm:p-6 lg:p-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800 tracking-tight">Laporan Surat Masuk</h2>
            <p class="text-sm text-slate-500 mt-1">Laporan rekapitulasi data surat masuk beserta filternya.</p>
        </div>
    </div>

    <!-- Filter Form -->
    <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
        <form method="GET" action="{{ route('laporan.surat-masuk') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            <div>
            ...
            <div>
                <label class="block text-xs font-semibold text-slate-700 mb-1">Pengirim</label>
                <select name="pengirim" class="w-full text-sm border-slate-300 rounded-lg focus:border-blue-500 focus:ring-blue-500">
                    <option value="">-- Semua Pengirim --</option>
                    @foreach($pengirimList as $p)
                        <option value="{{ $p }}" {{ request('pengirim') == $p ? 'selected' : '' }}>{{ $p }}</option>
                    @endforeach
                </select>
            </div>
            <div>
            ...
            <div class="md:col-span-4 flex gap-2 justify-end mt-2">
                <a href="{{ route('laporan.surat-masuk') }}" class="px-4 py-2 bg-slate-100 text-slate-600 hover:bg-slate-200 rounded-lg text-sm font-semibold transition-colors">Reset</a>
                ...
                <a href="{{ route('laporan.surat-masuk.pdf', request()->all()) }}" target="_blank" class="px-4 py-2 bg-emerald-600 text-white hover:bg-emerald-700 rounded-lg text-sm font-semibold transition-colors shadow-sm shadow-emerald-500/30 flex items-center gap-2">
