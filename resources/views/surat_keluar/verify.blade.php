...
<head>
    ...
    <title>Verifikasi Dokumen E-Sign - E-Office</title>
...
<body class="bg-slate-50 text-slate-800 min-h-screen flex flex-col justify-between">
        ...
        <div class="max-w-2xl w-full bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
            ...
            <div class="bg-slate-900 p-6 text-white text-center flex flex-col items-center justify-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-[10px] text-slate-400 uppercase tracking-widest mt-0.5">Sistem Verifikasi Dokumen Elektronik</p>
                </div>
            </div>

            <!-- Verification Status Banner -->
            <div class="p-8 text-center border-b border-slate-100">
                @if(strtolower($surat_keluar->status) === 'terkirim')
                    ...
                    <h2 class="text-xl font-bold text-emerald-800">DOKUMEN TERVERIFIKASI SAH</h2>
                    <p class="text-xs text-slate-500 mt-1.5">Tanda Tangan Elektronik (E-Sign) Terdaftar & Valid secara Hukum</p>
                @else
                    ...
                    <h2 class="text-xl font-bold text-amber-800">DOKUMEN PROSES APPROVAL</h2>
                    <p class="text-xs text-slate-500 mt-1.5">Tanda tangan elektronik belum lengkap atau masih dalam proses verifikasi internal.</p>
                @endif
            </div>

            <!-- Document Details -->
            <div class="p-8 space-y-6">
                <h3 class="text-sm font-bold text-slate-400 uppercase tracking-wider">Detail Informasi Dokumen</h3>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                    <div>
                        <span class="text-slate-400 block font-medium">Nomor Surat</span>
                        <span class="font-bold text-slate-800 mt-0.5 block">{{ $surat_keluar->nomor_surat }}</span>
                    </div>
                    <div>
                        <span class="text-slate-400 block font-medium">Tanggal Terbit</span>
                        <span class="font-semibold text-slate-800 mt-0.5 block">{{ \Carbon\Carbon::parse($surat_keluar->tanggal_surat)->locale('id')->translatedFormat('d F Y') }}</span>
                    </div>
                    <div class="md:col-span-2">
                        <span class="text-slate-400 block font-medium">Penerima / Tujuan</span>
                        <span class="font-semibold text-slate-800 mt-0.5 block">{{ $surat_keluar->tujuan }}</span>
                    </div>
                    <div class="md:col-span-2">
                        <span class="text-slate-400 block font-medium">Perihal</span>
                        <span class="font-semibold text-slate-800 mt-0.5 block">{{ $surat_keluar->perihal }}</span>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="p-4 rounded-2xl border {{ $surat_keluar->approvedDirut ? 'bg-emerald-50/30 border-emerald-100' : 'bg-slate-50/50 border-slate-100' }} flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                        <div class="flex items-start gap-3">
                            <div class="p-2 rounded-xl {{ $surat_keluar->approvedDirut ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-400' }} mt-0.5">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                            </div>
                            <div>
                                <h4 class="text-sm font-bold text-slate-800">
                                    {{ $surat_keluar->approvedDirut ? $surat_keluar->approvedDirut->name : 'Fredy Nuriat, S.Si.' }}
                                </h4>
                                <p class="text-xs text-slate-500">Direktur Utama</p>
                                @if($surat_keluar->approvedDirut)
                                    <p class="text-[10px] text-slate-400 mt-1">Pada: {{ \Carbon\Carbon::parse($surat_keluar->approved_dirut_at)->locale('id')->translatedFormat('d M Y - H:i') }} WIB</p>
                                @endif
                            </div>
                        </div>
                        <div>
    ...
    <div class="p-6 text-center text-xs text-slate-400 border-t border-slate-100 bg-white">
        ...
        Sistem Administrasi Surat Menyurat Elektronik (E-Office)
    </div>
